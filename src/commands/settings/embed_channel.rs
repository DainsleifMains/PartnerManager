...
async fn set(
	...
	if let Some(message_id) = current_message_id {
		...
		if let Err(error) = message_delete_result {
			ctx.send(CreateReply::default().content(format!("Updating the publish channel failed; the bot was unable to delete the message from the old channel. You will need to delete that message manually. (Error details: {})", error)).ephemeral(true)).await.into_diagnostic()?;
			return Ok(());
		}
	}
